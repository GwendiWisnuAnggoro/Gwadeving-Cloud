...
// CONFIG & STATE
// ==========================================
const APP_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbykiMleti8gpCdAqTlHy3ZQ083cDx9xXuuLvGSTwIv69K2C1cRur4QERkinaiFe23yO/exec";
const GAS_API_KEY = "GwadevingProduction";
const SPREADSHEET_ID = "1Gm7kCNLpI5nvbLVtea-Z7m8XSSgHeEXGgnGETmFH9EA";
...
function callGasAPI(action, payload = {}) {
    return new Promise((resolve) => {
        ...
        script.src = `${APP_SCRIPT_URL}?callback=${callbackName}&data=${encodeURIComponent(JSON.stringify(payload))}`;
...
async function callGasAPIFetch(action, payload = {}) {
    payload.action = action; payload.apiKey = GAS_API_KEY;
    try {
        const res = await fetch(`${APP_SCRIPT_URL}?data=${encodeURIComponent(JSON.stringify(payload))}`, { method: 'GET' });
...
function saveMetadataViaForm(payload, action = "save_metadata") {
    return new Promise((resolve) => {
        payload.action = action; payload.apiKey = GAS_API_KEY;
        const form = document.getElementById('hidden_upload_form');
        form.action = APP_SCRIPT_URL; document.getElementById('hidden_form_payload').value = JSON.stringify(payload);
        const frame = document.getElementById('hidden_upload_frame');
        frame.onload = () => resolve({ success: true });
        form.submit();
    });
}
...
// ==========================================
function gvizFetchSheet(sheetName) {
    return new Promise((resolve, reject) => {
        ...
        const timeoutId = setTimeout(() => {
            ...
            reject(new Error('Timeout membaca sheet ' + sheetName));
        }, 10000);
        window[cbName] = function (resp) {
            if (isDone) return; isDone = true; clearTimeout(timeoutId); delete window[cbName];
            try {
                if (!resp || resp.status === 'error' || !resp.table) { reject(new Error('gviz error pada sheet ' + sheetName)); return; }
                const rows = (resp.table.rows || []).map(r => (r.c || []).map(cell => (cell && cell.v !== undefined && cell.v !== null) ? cell.v : ''));
                resolve(rows);
            } catch (err) { reject(err); }
        };
        const script = document.createElement('script');
        script.src = `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/gviz/tq?tqx=out:json;responseHandler:${cbName}&sheet=${encodeURIComponent(sheetName)}&headers=1&_=${Date.now()}`;
        script.onerror = () => { if (isDone) return; isDone = true; clearTimeout(timeoutId); delete window[cbName]; reject(new Error('Gagal memuat sheet ' + sheetName)); };
        script.onload = () => { if (script.parentNode) script.remove(); };
        document.body.appendChild(script);
    });
}

function decryptFileRows_(rows) {
    ...
    // menerima data plaintext tanpa perlu tahu soal enkripsi.
    return rows.map(r => {
        const copy = r.slice();
        if (copy.length > 1) copy[1] = decField(copy[1]);
        if (copy.length > 2) copy[2] = decField(copy[2]);
        if (copy.length > 6) copy[6] = decField(copy[6]);
        if (copy.length > 7) copy[7] = decField(copy[7]);
        return copy;
    });
}
...
async function fetchRawSheets() {
    const [sheet1Rows, trashRows, folderRows, sharesRows] = await Promise.all([
        gvizFetchSheet('DB_01'), 
        gvizFetchSheet('DB_02'), 
        gvizFetchSheet('DB_03'), 
        gvizFetchSheet('DB_05')
    ]);
    return {
        sheet1Rows: decryptFileRows_(sheet1Rows),
        trashRows: decryptFileRows_(trashRows),
        folderRows,
        sharesRows
    };
}
...
function renderFoldersAndFiles(filterText = '') {
    ...
    if (subFolders.length === 0) { document.getElementById('folder-container-wrapper').style.display = 'none'; }
    else {
        ...
        subFolders.forEach(fullFolderPath => {
...
function renderFolderTabOnly(filterText = '') {
    ...
    folderList.innerHTML = "";
    if (all.length === 0) { folderList.innerHTML = `<div style="grid-column:1/-1; text-align:center; padding:60px 20px; color:var(--text-muted); font-size:14px;">Belum ada folder.</div>`; return; }
    all.forEach(fullFolderPath => {
        const stats = getFolderStats(fullFolderPath);
...
function renderFiles(filterText = '', mode = 'home') {
    ...
    if (data.length === 0) {
        list.innerHTML = `<div style="grid-column: 1/-1; text-align:center; padding: 60px 20px; color: var(--text-muted); font-size:14px;">Belum ada berkas di sini.</div>`;
        return;
    }
...
async function parseAndShowExif(urlOrBlob, container) {
    if (!window.exifr) { container.innerHTML = `<div style="margin-top:12px; font-size:12px; color:var(--text-muted);">Library EXIFR belum dimuat.</div>`; return; }
    try {
...
async function startMultiUpload() {
    ...
    while (uploadQueue.length > 0) {
        ...
        if (sharedUploadActive) {
            const res = await saveMetadataViaForm({ fileId: uniqueId, customName: customName, originalName: fileInput.name, format: format, folder: folderTarget, fileSize: fileInput.size, totalChunks: totalChunks, chunks: uploadedChunks, thumbId: thumbTelegramId, shareId: sharedUploadShareId, viewerUid: sharedUploadViewerUid }, 'shared_save_metadata');
            if (!sharedContentsCache.files.some(f => f.id === uniqueId)) sharedContentsCache.files.push({ id: uniqueId, name: customName, originalName: fileInput.name, format: format, folder: folderTarget, size: fileInput.size, thumbId: thumbTelegramId });
        } else {
            await saveMetadataViaForm({ fileId: uniqueId, customName: customName, originalName: fileInput.name, format: format, folder: folderTarget, fileSize: fileInput.size, totalChunks: totalChunks, chunks: uploadedChunks, thumbId: thumbTelegramId, ownerId: currentOwnerId });
            if (!stateArray.active.some(f => f.id === uniqueId)) stateArray.active.push({ id: uniqueId, name: customName, originalName: fileInput.name, format: format, folder: folderTarget, size: fileInput.size, thumbId: thumbTelegramId });
        }
...
async function renderPreviewContent(url, ext, container, blob) {
    ...
    try {
        if (IMAGE_EXTENSIONS.has(ext)) {
            container.innerHTML=`<div style="display:flex;align-items:center;justify-content:center;width:100%;height:100%;"><img class="preview-media" src="${url}" alt="Preview" style="max-width:100%; max-height:85vh; object-fit:contain; border-radius:10px;"></div>`; 
            return;
        }
        if (VIDEO_EXTENSIONS.has(ext)) {
